...
const verificationTasks = [
  {
    id: 1,
    patient: 'Nara',
    therapist: 'Terapis Naya',
    type: 'mismatch',
    plan: { method: 'Transfer', icon: Landmark },
    actual: { method: 'Cash', icon: Banknote },
  },
  {
    id: 2,
    patient: 'Arka',
    therapist: 'Terapis Sari',
    type: 'pending',
    plan: { method: 'Transfer', icon: Landmark },
    actual: { method: 'Transfer', icon: Landmark },
  }
];
    ...
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="space-y-4">
        {verificationTasks.map((task) => (
          ...
          >
                  ...
                  <div className="grid grid-cols-2 gap-8">
                     <div>
                        ...
                        <div className="flex items-center gap-2 text-on-surface-variant font-bold text-sm">
                           <task.plan.icon className="w-4 h-4 opacity-60" />
                           {task.plan.method}
                        </div>
                     </div>
                     <div>
                        ...
                        <div className={cn(
                          ...
                          task.type === 'mismatch' ? "text-error" : "text-on-surface"
                           ...
                           <task.actual.icon className="w-4 h-4" />
                           {task.actual.method}
                        </div>
                     </div>
                  </div>
             ...
             <div className="flex md:flex-col items-center justify-between md:justify-center gap-3 pt-4 md:pt-0 border-t md:border-t-0 border-surface-container">
                ...
                <button className="bg-primary-container text-on-primary-container font-bold px-8 py-3 rounded-2xl hover:brightness-110 active:scale-95 transition-all text-sm shadow-sm md:w-full">
                  Verify
                </button>
             </div>
          </motion.div>
        ...
        <div className="text-center py-12 opacity-50 space-y-2">
           ...
           <p className="text-sm font-medium">Showing {verificationTasks.length} verification tasks.</p>
        </div>
      </div>
    </div>
  );
