...
export default function MasterData() {
  ...
  // 🟢 Form State Dinamis (Ditambah user_id)
  const [formData, setFormData] = useState({
    nama_layanan: '', harga_saat_ini: '', persentase_komisi: '', 
  ...
  const openModal = (mode: 'create' | 'edit', data: any = null) => {
    ...
    if (mode === 'edit' && data) {
      ...
      setFormData({ nama_layanan: '', harga_saat_ini: '', persentase_komisi: '', user_id: '', nama_terapis: '', no_whatsapp: '', status_aktif: '1' });
    }
    setIsModalOpen(true);
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {dbServices.map((service, i) => {
            ...
            return (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: i * 0.05 }} key={service.id} className="bg-surface-container-lowest p-6 rounded-3xl border border-surface-container-high shadow-sm relative group hover:shadow-xl hover:border-primary-container/20 transition-all overflow-visible">
                <div className="flex justify-between items-start mb-6">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center transition-transform group-hover:rotate-12 bg-primary-container/10 text-primary-container"><ServiceIcon className="w-6 h-6" /></div>
                ...
                <div className="absolute bottom-6 right-6">
                   <button onClick={() => setActiveMenuId(activeMenuId === service.id ? null : service.id)} className="p-2 rounded-full hover:bg-surface-container transition-all"><MoreVertical className="w-4 h-4 text-on-surface-variant" /></button>
